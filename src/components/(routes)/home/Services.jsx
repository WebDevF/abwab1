    ...
    <div className="w-full mb-5">
      <div className="w-full flex flex-col items-center text-center gap-4 px-4 sm:px-6 lg:px-8 py-5 sm:py-8 lg:mt-10">
        <div className="flex items-center gap-2 text-sm sm:text-base font-normal text-[#0A40C6] bg-[#E7ECF9] px-3 sm:px-4 py-2 rounded-full w-fit">
          <img src="/assets/img/services/Layer_x0020_1 (1).svg" className="w-6 h-6 sm:w-5 sm:h-5" alt="" />
          <span className="text-[1rem] sm:text-[1rem] lg:text-[1.4rem] font-normal text-[#0A40C6]">{t("services.badge")}</span>
        </div>
        ...
        <div className="block lg:hidden">
          ...
          <div className="flex flex-col gap-3 sm:gap-4">
            {buttons.map((text, index) => (
              <button
                key={index}
                className={`text-right text-base sm:text-lg font-normal px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl transition-all duration-200 ${
        ...
        <div className="hidden lg:block">
          <div className="grid grid-cols-12 gap-6 xl:gap-8 2xl:gap-12 items-center min-h-[600px] xl:min-h-[580px]">
            <div className="col-span-12 lg:col-span-3 xl:col-span-3">
              <div className="flex flex-col gap-7 xl:gap-7 2xl:gap-10">
                {buttons.map((text, index) => (
                  <button
                    key={index} 
                    className={`text-right text-[1.2rem]  xl:text-[1.3rem] 2xl:text-[1.7rem] font-[700] px-5 xl:px-8 py-4 xl:py-5 2xl:py-7 rounded-2xl xl:rounded-xl transition-all duration-200 whitespace-nowrap cursor-pointer ${
                      selectedImage === images[index].img
            ...
            <div className="col-span-12 lg:col-span-6 xl:col-span-6">
              <div className="flex justify-center">
                <img
                  src={selectedImage || "/placeholder.svg"} 
                  alt="Selected service"
                  className="w-full  h-[510px] xl:h-[530px] 2xl:h-[642px] object-cover rounded-3xl xl:rounded-[1rem] shadow-lg transition-all duration-300"
                />
              </div>
            </div>
